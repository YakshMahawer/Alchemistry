...
const Result = (props) => {
  ...
  const chemh =
    "M 218.985 165.204 V 384.283 C 218.985 397.931 232.87 409 250.003 409 C 267.136 409 281.02 397.935 281.02 384.283 V" +
    (387.28 -
  ...
  const mobileNumber = "+919265140199";
  const encodedMobileNumber = encodeURIComponent(mobileNumber);
  useEffect(() => {
    fetch(
      `/result/${location.state.chemA}/${location.state.chemB}/${location.state.chemC}/${location.state.chemD}?mobileNumber=${encodedMobileNumber}`
    )
        ...
        <div className="result_lab">
          ...
          {data.map((item, index) => (
            <div className="result_section">
              ...
              <div className="note">
                NOTE: All the results are revelent for normal temperature.
              </div>
              <div className="result" key={index}>
                ...
                <div className="testube_info">
                  <div className="result_smell">
                    <img src={cloud} alt="" />
                    <div className="result_smell_text">
                      {location.state.chemA > 0
                        ? "Pungent Smell Due to HCl"
                        : "No Specific Smell"}
                    </div>
                  </div>
                  <div className="result_equation">{item.result}</div>
                  <div className="resultant_testtube">
                    <ResultCustomTestTube
                      color={item.color}
